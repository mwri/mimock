...
let mimock_method = (function () {
	...
	mimock_method.prototype.wrap = function (wrap_fun) {
		...
		return new mimock_restorable(this, () => { this.unwrap(wrap_fun); return this; });
	...
	mimock_method.prototype.unwrap = function (wrap_fun) {

		let index = this.wrap_chain.indexOf(wrap_fun);

		if (index === -1)
			return 0;

		this.wrap_chain.splice(index, 1);

		return 1;

	};

	mimock_method.prototype.uw = mimock_method.prototype.unwrap;
...
let mimock_fun = (function () {
	...
	mimock_fun.prototype.unwrap = function (wrap_fun) {
	...
	mimock_fun.prototype.uw = mimock_fun.prototype.unwrap;
...
let mimock_export = (function () {
	...
	mimock_export.prototype.wrap = function (wrap_fun) {
		...
		for (let i = 0; i < this.instances.length; i++)
			this.instances[i].wrap.wrap(wrap_fun);
	...
	mimock_export.prototype.unwrap = function (wrap_fun) {

		let index = this.wrap_funs.indexOf(wrap_fun);

		if (index === -1)
			return 0;

		this.wrap_funs.splice(index, 1);

		let unwrap_count = 0;
		for (let i = 0; i < this.instances.length; i++)
			unwrap_count += this.instances[i].wrap.unwrap(wrap_fun);

		return unwrap_count;

	};

	mimock_export.prototype.uw = mimock_export.prototype.unwrap;
...
let mimock_restorable = (function () {


	let mimock_restorable = function restorable (obj, restore_fun) {

		this.obj         = obj;
		this.restore_fun = restore_fun;

	};


	mimock_restorable.prototype.restore = function () {
